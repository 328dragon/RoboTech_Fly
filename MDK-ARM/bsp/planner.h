...
public:
    ...
    SimpleStatus_t &LoactaionCloseControl(const odom_t &target_odom, float max_linear, float max_angular, const odom_t &target_error = {0.1, 0.4, 0.5}, bool clearodom = false);
...
private:
    ...
    float _max_linear_acc =2.8f; // 最大线加速度
    float _max_linear_dec = 2.0f; // 最大线减速度
    float _max_angular_acc =4.0f; // 最大角加速度
